...
//*************************************************************************************************
//
// Форма состояния параметров АКБ
//
//*************************************************************************************************
...
#define WIN_SIZE_Y      370

//*************************************************************************************************
// Определение идентификаторов объектов в форме
//*************************************************************************************************
#define ID_FRAMEWIN_0   (GUI_ID_USER + 3)

#define ID_BUTTON_CLS   (GUI_ID_USER + 0x10 + 2)
...
#define ID_NAME_18      (GUI_ID_USER + 0x20 + 18)
...
#define ID_VALUE_18     (GUI_ID_USER + 0x40 + 18)
...
#define ID_UNIT_18      (GUI_ID_USER + 0x80 + 18)
...
static const GUI_WIDGET_CREATE_INFO DialogCreate[] = {
    { FRAMEWIN_CreateIndirect, "Монитор АКБ", ID_FRAMEWIN_0, WIN_POS_X( WIN_SIZE_X ), WIN_POS_Y( WIN_SIZE_Y ), WIN_SIZE_X, WIN_SIZE_Y },
    ...
    { TEXT_CreateIndirect, "", ID_NAME_18, TEXT_NAME_POS_X, TEXT_NAME_POS_Y( 19 ), TEXT_NAME_SIZE_X, TEXT_NAME_SIZE_Y },
    ...
    { TEXT_CreateIndirect, "", ID_VALUE_18, TEXT_VALUE_POS_X, TEXT_NAME_POS_Y( 19 ), TEXT_VALUE_SIZE_X, TEXT_VALUE_SIZE_Y },
    ...
    { TEXT_CreateIndirect, "", ID_UNIT_18, TEXT_UNIT_POS_X, TEXT_NAME_POS_Y( 19 ), TEXT_UNIT_SIZE_X, TEXT_UNIT_SIZE_Y }
 };
...
static void UpdateWidget( WM_HWIN hwnd ) {
    ...
    i = DevParamCnt( ID_DEV_BATMON, CNT_HMI );
    for ( id = 0; id < i; id++ ) {
        rel_id = DevParamRelat( ID_DEV_BATMON, id );
        TEXT_SetText( WM_GetDialogItem( hwnd, ID_VALUE_00 + id ), ParamGetForm( ID_DEV_BATMON, rel_id, PARAM_VALUE ) );
       }
 }
...
static void cb_Dialog( WM_MESSAGE *pMsg ) {
    ...
    switch ( pMsg->MsgId ) {
        case WM_INIT_DIALOG:
            ...
            for ( id = 0; id < DevParamCnt( ID_DEV_BATMON, CNT_HMI ); id++ ) {
                rel_id = DevParamRelat( ID_DEV_BATMON, id );
                //вывод наименования параметра
                TEXT_SetText( WM_GetDialogItem( pMsg->hWin, ID_NAME_00 + id ), ParamGetForm( ID_DEV_BATMON, rel_id, PARAM_DESC ) );
                //вывод единиц измерения параметра
                TEXT_SetText( WM_GetDialogItem( pMsg->hWin, ID_UNIT_00 + id ), ParamGetForm( ID_DEV_BATMON, rel_id, PARAM_UNIT ) );
               }
...
void WinBatMon( void ) {
    ...
    GUI_ExecDialogBox( DialogCreate, GUI_COUNTOF( DialogCreate ), &cb_Dialog, WM_HBKWIN, 0, 0 );
 } 
